    ...
    const slideshowContainer = {
        ...
        load (data) {
            if (data && data != undefined) {
                ...
                const canvas = slideshowContainer.canvas;
                let asteroidGroup = canvas.select("g");
                const earthGroup = canvas.select("g:nth-child(2)");
                ...
                moveAsteroid(); // start animation loop
                ...
                ;
                document.getElementById("nasa-slideshow").classList.add("data-loaded");
                return true;
            }
        },
        unload () {
            const canvas = slideshowContainer.canvas;
            canvas.select("g").selectAll("g").transition(); // stop animation (animation events goes ON even if the objects are removed...)
            canvas.select("g").selectAll("g").remove();
            document.getElementById("nasa-slideshow").classList.remove("data-loaded");
            return true;
        }
    };
